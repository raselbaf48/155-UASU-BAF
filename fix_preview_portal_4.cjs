...
const badStr = `      {/* Print Preview Portal */}
...
// Just doing a clean rebuild of the file ending
const endRegex = /<\/div>\s*<\/div>\s*<\/div>\s*(\{\/\* Print Preview Portal \*\/[\s\S]*?)?\}\s*<\/div>\s*\);\s*\};\s*$/;
...
// Find the start of the DutyDetailsModal
...
if (detailsModalStartIdx !== -1) {
    ...
    // Clean up the modal part manually
    // The modal part should end with:
    //         </div>
    //       </div>
    //     </div>
    //   );
    ...
    const cleanModalPart = modalPart.replace(/\{\/\* Print Preview Portal \*\/[\s\S]*?$/, '');
    // Need to make sure we close the main div, return the portal, then close the React component
    const lastDivIndex = cleanModalPart.lastIndexOf('</div>');
    let finalPart = cleanModalPart.substring(0, lastDivIndex);
    // actually, let's just strip everything after the table block and rebuild the end
