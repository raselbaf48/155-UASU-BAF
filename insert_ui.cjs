const fs = require('fs');
const lines = fs.readFileSync('src/components/AirmanProfileModal.tsx', 'utf8').split('\n');
...
const newUI = `
                      ...
                      ) : editingGroup[0].dutyCode === 'DEPLOYMENT' || editingGroup[0].dutyCode === 'ATT' ? (
                        <>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="block text-[11px] font-bold text-slate-600 dark:text-slate-400 mb-1 uppercase tracking-wider">From Date</label>
                          ...
                          {editingGroup[0].dutyCode === 'DEPLOYMENT' ? (
                              <div>
                                ...
                                <div className="flex flex-wrap gap-1.5 mb-2">
                                   {/* Note: In AirmanProfileModal presetDeployLocations may not be defined. Wait, it's defined in AssignDeploymentTab. Let's just use presetLocations for now or skip it. */}
                                   {presetLocations.map(loc => (
                                      <button
                                        key={loc}
                                        type="button"
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={editingGroup[0].dutyCode === 'DEPLOYMENT' ? editDepRemarks : editNotes}
                              onChange={(e) => {
                                  if (editingGroup[0].dutyCode === 'DEPLOYMENT') setEditDepRemarks(e.target.value);
...
const startIndex = lines.findIndex(l => l.includes('{editingGroup[0].dutyCode === \'LEAVE\' ? ('));
const endIndex = lines.findIndex((l, i) => i > startIndex && l.includes('<div className="flex items-center gap-3 pt-2">'));
...
// Ensure editDepLocation and presetDeployLocations exist in AirmanProfileModal.tsx
// wait, I can just use presetLocations for both TDY and Deployment inside AirmanProfileModal since it's an editor.

lines.splice(startIndex, endIndex - startIndex, newUI);
fs.writeFileSync('src/components/AirmanProfileModal.tsx', lines.join('\n'));
