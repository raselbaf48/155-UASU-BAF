...
export const PosSales: React.FC = () => {
  ...
  const removeHistoryItem = async (txId: string) => {
      ...
      alert("Entry removed and member BAKI reversed successfully.");
  };
  ...
  const handleCheckout = async () => {
      ...
      alert(`Sale completed successfully for ৳${memberChargeAmount * multiplier}!`);
      setBasket([]);
      setSelectedMembers([]);
      setMemberSearchTerm('');
      fetchCatalog();
         ...
         <div className="bg-slate-900 rounded-[2rem] border-t-4 border-t-[#4f46e5] shadow-sm border border-slate-800 min-h-[600px] flex flex-col sticky top-24">
            ...
            <div className="p-6 border-b border-slate-800 relative z-10 space-y-4">
                ...
                <div className="relative">
                   ...
                   {showMemberDropdown && (
                       <div className="absolute left-0 right-0 top-full mt-2 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-60">
            ...
            <div className="p-6 bg-slate-800 rounded-b-[2rem] border-t border-slate-800 space-y-4">
                ...
                <button 
                    ...
                    className={`w-full py-4 rounded-xl text-[10px] font-black tracking-widest uppercase flex items-center justify-center space-x-2 transition-all shadow-md ${basket.length > 0 ? 'bg-emerald-900/300 hover:bg-emerald-600 text-white shadow-emerald-500/20' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}
                >
